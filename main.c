...
int main(int argc, const char *argv[]) {
  ...
  int old_file_size = mapfile(argv[1], (void*) &old_file_data);
  if (old_file_size < 0)
    exit(EXIT_FAILURE);

  int new_file_size = mapfile(argv[2], (void*) &new_file_data);
  if (new_file_size < 0)
    exit(EXIT_FAILURE);

  int fd = open(argv[3], O_CREAT | O_WRONLY | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
